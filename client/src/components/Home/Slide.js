...
const Slide = ({ products ,title })=> {
  ...
  return (
    ...
    <Box>
        <Box>
            <Typography style={{fontFamily:'Times New Roman',fontWeight:600,marginLeft:'50px',color:'blueviolet'}} variant='h5'>{title}</Typography>
        </Box>
        <Divider/>
    <Carousel 
    ...
    >
        {
            products.map(post=>(
                title=== post.categories &&
                    <Link  to={`/current/${post._id}`}  style={{padding:'10px',marginTop:'10px',textAlign:'center',textDecoration:'none'}}>
